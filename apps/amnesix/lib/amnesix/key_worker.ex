defmodule Amnesix.KeyWorker do
  ...
  defmodule State do
    defstruct [:state, :timer, :work_items, :next_work_item,
               :persister_mod, :persister_pid, :persister_key]
  end

  # Front-end

  def start_link(persister, persister_key) do
    GenServer.start_link(__MODULE__, [persister, persister_key])
  end
  ...
  def init([_persister = {persister_mod, persister_pid}, persister_key]) do
    {:ok, %State{state: :initializing,
                 work_items: %{},
                 persister_mod: persister_mod,
                 persister_pid: persister_pid,
  ...
  defp persist(state = %State{persister_mod: nil}), do: state
  defp persist(state) do
    # All we need to persist is work items, we can rebuild the rest from there
    :ok = state.persister_mod.persist(state.persister_pid, state.persister_key, state.work_items)
    state
  end
